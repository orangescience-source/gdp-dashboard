...
def render_thumbnail_tab():
    ...
    with st.expander("⚠️ 유튜브 썸네일 가이드라인 안내 (업로드 전 반드시 확인)", expanded=False):
        st.markdown(
            """
            **생성된 프롬프트는 유튜브 커뮤니티 가이드라인을 준수하도록 설계되었습니다.**
            업로드 전 아래 항목을 최종 확인하세요.

            | 항목 | 기준 |
            |------|------|
            | ❌ 선정적·성적 표현 | 노출, 성적 자세, 암시적 이미지 절대 금지 |
            | ❌ 폭력·유혈 | 과도한 피, 부상, 공포 장면 금지 |
            | ❌ 실존 인물 허위 묘사 | 공인 합성 오해 유발 금지 |
            | ❌ 클릭베이트 허위 이미지 | 영상 내용과 무관한 자극적 이미지 금지 |
            | ⚠️ 과도한 표정 과장 | 자연스러운 감정 표현 수준 유지 권장 |
            | ✅ 감정 표현 | 놀람·호기심·진지함 수준은 허용 |

            위반 시 **영상 삭제, 수익 제한, 채널 경고** 등의 제재가 발생할 수 있습니다.
            자세한 내용은 [유튜브 커뮤니티 가이드라인](https://www.youtube.com/howyoutubeworks/policies/community-guidelines/)을 참고하세요.
            """,
