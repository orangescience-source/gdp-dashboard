...
def render_analysis_tab():
    ...
    c1.metric("분석 영상 수", f"{result['total_count']}개")
    c2.metric("니치 영상 수", f"{result['niche_count']}개")
    c3.metric("평균 조회수", f"{avg_views:,.0f}")
    niche_ratio = (
        result["niche_count"] / result["total_count"] * 100
        if result["total_count"] > 0
        else 0
    )
    c4.metric("니치 비율", f"{niche_ratio:.1f}%")
