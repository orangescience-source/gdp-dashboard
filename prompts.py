...
① 스타일 고정:
   ...
   'Korean webtoon style 2D illustration,
   Japanese anime flat color style,
   bold black outlines 3.5px,
   NO photorealism, NO realistic skin texture,
   NO 3D CGI render, NO photograph,
   cartoon illustration only,'

② 채널 캐릭터 자동 주입:
   채널별 주인공을 아래 형식으로 포함한다.

   거침없는 경제학:
   'protagonist Tae-oh wearing vivid red mask
   covering upper face, sharp black formal suit,
   aggressive warning pose,'

   머니매커니즘:
   'protagonist Gear Yoon wearing gold-rimmed
   glasses, emerald eyes, dark blazer,
   cold analytical expression,'

   친절한 경제학자:
   'protagonist Saito with neat black hair,
   horn-rimmed glasses, navy jacket,
   confident mentor smile,'

   남몰래 경제학:
   'protagonist Shadow wearing purple mask,
   trench coat, giant magnifying glass,
   mysterious crouching pose,'

   사이언스로그:
   'protagonist Log wearing blue visor HUD,
   high-tech bodysuit, data scanner prop,'

   사이언스툰:
   'protagonist Nutty with oversized goggles,
   wild hair, colorful lab coat,
   exaggerated shocked expression,'

   미래인사이트:
   'protagonist Future with cybernetic left eye,
   neon-trimmed jacket, hologram wrist display,'

   히스토리프로파일러:
   'protagonist Lupus wearing brown fedora,
   trench coat, evidence file prop,'

   친절한 심리학자:
   'protagonist Psy wearing mint sweatshirt,
   semi-rimless glasses, warm empathetic smile,'

   거리의 경제학:
   'protagonist Hyeonjang wearing orange beanie,
   carrying shopping basket, energetic pose,'

   친절한 공학자:
   'protagonist Engi wearing white hard hat,
   blue work uniform, blueprint roll prop,'

   친절한 과학자:
   'protagonist Scien with brown hair,
   round glasses, white lab coat,
   warm curious expression,'

   친절한 사회학자:
   'protagonist Socio wearing round glasses,
   brown cardigan, thoughtful observer pose,'

③ 출력 형식:
   image_prompts 배열의 각 항목은
   아래 순서로 구성한다:
...
④ 썸네일 수:
   image_prompts 는 반드시 3종 이상 생성한다.
...
[역할]
당신은 대한민국 유튜브 시장 Top 1% 시나리오 작가입니다.
TTS(Text-to-Speech)에 바로 입력 가능한
순수 구어체 대본을 집필하는 것이 임무입니다.
...
썸네일 문구: {confirmed_thumbnail}
초반 30초 Hook 전략: {hook_30sec}

[이번 생성 범위: 앞부분]
아래 6개 구간의 앞부분을 작성한다.
HOOK → TEASER → BIG IDEA → INTRO → BODY 1 → BODY 2
각 구간의 흐름 가이드:

HOOK (약 500자)
- 썸네일 약속을 처음 20초 안에 구체적 상황·증거·반응으로 구현
- 이 영상을 끝까지 보면 얻을 핵심 보상을 명확히 예고
- 예시: "이 영상을 끝까지 보시면 ___를 바로 실천할 수 있습니다"
- 절대 썸네일 문구를 그대로 읽지 말 것

TEASER (약 500자)
- 이 주제가 왜 지금 중요한지 공감 유도
- 시청자의 현실 문제와 연결

BIG IDEA (약 500자)
- 이 영상의 핵심 인사이트를 한 문장으로 선언
- 구체적 수치나 사례로 뒷받침
...
핵심 주제: {body2_topic}
- BODY 1과 이어지는 흐름 유지
- 장면 묘사 → 인물 반응 → 데이터 → 해석 순환
- 미니훅으로 마무리: {mini_hook1_sentence}

[절대 금지 출력 형식]
다음 요소는 단 하나도 출력하지 않는다:
- ## 헤더 (예: ## HOOK, ## BODY 1)
- 타임코드 (예: [00:00], [04:00])
- 마크다운 기호 (**, *, #, -, >)
- 괄호 지문 (예: (카메라가 천천히 줌인), [장면: ...])
- 시각화 메모 (DATA_SKETCH_SCENE 등)
- 섹션 라벨 (HOOK:, BODY1: 등)
오직 말할 수 있는 구어체 문장만 출력한다.
단락 구분은 빈 줄 1개만 사용한다.

[숫자·연도 표기 규칙 — TTS 필수]
모든 숫자와 연도는 한글 독음으로 표기한다:
2025년 → 이천이십오 년
1998년 → 천구백구십팔 년
10% → 십 퍼센트
3.5% → 삼 점 오 퍼센트
...
3명 → 세 명
2배 → 두 배
단, 채널명·브랜드명·고유명사는 그대로 유지한다.

[분량 기준]
앞부분 총 5,000자 이상 작성한다.
분량이 부족하면 멈추지 말고 즉시 아래 방법으로 보완하며 계속 작성한다:
① 장면·공간·인물 반응 묘사를 3~4문장으로 확장
② 구체적 사례나 실제 상황을 1개 이상 추가
③ 등장인물 감정·행동·대사 1~2문장 추가
④ 수치가 나오면 쉬운 비교로 2~3문장 풀어서 추가
⑤ 개념 설명 직후 비유 문장 1개 추가

[난이도 원칙 — 반드시 준수]
초등학생도 이해할 수 있도록 쉬운 어휘와 친절한 설명을 사용한다.
복잡한 개념이나 전문 용어가 등장할 경우
반드시 바로 다음 문장에서 쉬운 말로 풀어 설명한다.
설명 없이 전문 용어만 단독으로 사용하는 것은 절대 금지한다.

[품질 자가검증 체크리스트]
출력 전 반드시 확인:
✅ 헤더·타임코드가 단 하나도 없는가?
✅ 모든 숫자가 한글 독음인가?
✅ 전문 용어 뒤에 쉬운 설명이 이어지는가?
✅ 앞부분 5,000자 이상인가?
✅ 오직 구어체 문장만인가?
"""
...
[이번 생성 범위: 뒷부분]
앞부분(HOOK~BODY 2)에 이어서 뒷부분을 작성한다.
BODY 3 → BODY 4 → REVEAL → IMPACT → END
앞부분 마지막 문장과 자연스럽게 연결되어야 한다.

BODY 3 (약 1,600자)
핵심 주제: {body3_topic}
- 앞부분과 끊김 없이 이어지는 흐름 시작
- 미니훅 포함: {mini_hook2_sentence}

BODY 4 (약 1,600자)
핵심 주제: {body4_topic}
- 핵심 증거·데이터·사례 최대 집중
- 미니훅 포함: {mini_hook3_sentence}
...
[분량 기준]
뒷부분 총 5,000자 이상 작성한다.
분량 부족 시 앞부분과 동일한 보완 방법으로 즉시 계속 작성한다.

[난이도 원칙 — 반드시 준수]
초등학생도 이해할 수 있도록 쉬운 어휘와 친절한 설명을 사용한다.
전문 용어는 반드시 바로 다음 문장에서 쉬운 말로 풀어 설명한다.

[뒷부분 완료 후 — 시각화 연동 메모 추가]
뒷부분 대본 작성 완료 후 반드시 아래 형식으로 메모를 추가한다:
...
Strictly NO photorealism.
Strictly NO realistic skin texture.
Strictly NO photograph-like rendering.
Strictly NO 3D CGI render style.
Strictly NO hyperrealistic shading.
Strictly NO real human faces.

반드시 아래 스타일로만 생성:
- Korean webtoon 2D illustration style
- Japanese anime flat color style
- Bold cartoon illustration style

모든 이미지 프롬프트 문장 안에
아래 문구를 반드시 포함한다:
'Korean webtoon style 2D cartoon illustration,
NO photorealism, NO realistic rendering,
NO 3D CGI, bold black outlines 3.5px,
flat colors, high contrast cell shading,
cartoon style only.'
