...
STAGE 1 [00:00~01:30] HOOK
  ...
  도입부 비유 사용 원칙 (필수):
  초반 1~2분 안에 반드시 핵심 개념을 설명하는 비유 1개를 아래 기준에 맞게 삽입한다.
  ...
  좋은 비유 예시 (채널별):
  [경제 채널]
  '금리가 오른다는 건 돈의 가격이 비싸진다는 뜻입니다.
  마치 놀이공원 입장료가 갑자기 두 배로 오르면 사람이 줄어드는 것처럼,
  돈이 비싸지면 사람들이 빌리는 걸 줄이게 됩니다.'
  [과학 채널]
  '블랙홀은 우주에서 가장 욕심쟁이입니다. 한번 잡으면 빛조차 놓아주지 않거든요.
  마치 아무리 힘껏 당겨도 절대 빠져나올 수 없는 강력한 자석처럼요.'
  [심리 채널]
  '자존감이 낮은 사람은 배터리가 항상 10%인 스마트폰과 같습니다.
  켜져 있긴 한데 제대로 작동을 못해요.'
  [역사 채널]
  '역사는 반복됩니다. 마치 유행처럼요.
  70년대 패션이 지금 다시 뜨듯, 경제 위기도 같은 패턴으로 반복해서 찾아옵니다.'

  비유 작성 금지:
  - 전문 용어를 비유 안에 그대로 쓰는 것
  - 비유가 너무 길어서 핵심이 흐려지는 것
  - 억지스럽거나 연결이 부자연스러운 것
  - '마치 ~처럼' 형식 없이 그냥 설명만 하는 것

  비유 삽입 위치:
  - HOOK 또는 TEASER 구간 내, 핵심 개념이 처음 등장하는 직후
  - 시청자가 '이게 뭐지?' 라고 느끼는 순간 바로 다음 문장에 배치
...
[분량 자가 검증 규칙] (필수)
대본 생성 완료 후 반드시 아래 기준으로 자가 검증한다.

FRONT (앞부분) 기준:
- HOOK + TEASER + BIG IDEA + INTRO + BODY 1 + BODY 2 합산
- 목표 분량: 5,000자 이상
- 부족 시: 아래 보완 방법 즉시 적용

전체 합산 기준:
- FRONT + BACK = 10,000자 이상
- 최고 품질 목표: 12,000자 이상

분량 부족 시 보완 방법 (아래 순서대로 적용):
① 장면 묘사 확장
   각 BODY 청크에서 장면·공간·인물 반응 묘사가 2문장 미만이면 3~4문장으로 확장
② 사례 추가
   각 BODY 청크에 구체적 사례나 실제 상황 묘사를 1개 이상 추가
③ 대조 인물 반응 추가
   등장인물의 감정·행동·대사를 1~2문장 추가
④ 데이터 설명 확장
   수치나 통계가 등장하면 그 의미를 초등학생 수준으로 풀어서 2~3문장 추가
⑤ 비유 삽입
...
BACK (뒷부분) 기준:
- BODY 3 + BODY 4 + REVEAL + IMPACT + END 합산
- 목표 분량: 5,000자 이상
- 부족 시: 아래 보완 방법 즉시 적용
...
[시각화 검증]
✅ 각 2~4문장마다 시각 정보가 있는가?
✅ DATA_SKETCH_SCENE 후보 구간을 메모로 남겼는가?

검증 결과 미달 항목 발견 시:
해당 섹션을 즉시 보완하고 다시 검증을 반복한다.
모든 항목 ✅ 통과 후에만 최종 출력으로 제출한다.
