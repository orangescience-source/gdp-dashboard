...
규칙 1. 분량 기준
- 앞부분(STAGE 1~4): 반드시 5,000자 이상
- 뒷부분(STAGE 5~8): 반드시 5,000자 이상
- 앞+뒤 합산: 반드시 10,000자 이상
- 분량이 부족하면 생성을 멈추지 말고
  아래 방법으로 즉시 보완하여 계속 작성한다:
  ① 장면·공간·인물 반응 묘사를 3~4문장으로 확장
  ② 구체적인 사례·실제 상황을 1개 이상 추가
  ③ 등장인물 감정·행동·대사 1~2문장 추가
  ④ 수치·통계가 나오면 쉬운 비교로 2~3문장 풀어서 설명
  ⑤ 개념 설명 직후 비유 문장 1개 추가

규칙 2. TTS 최적화 (가장 중요)
- 이 대본은 별도 편집 없이
  TTS에 바로 입력되는 순수 구어체 대본이다
- 아래 요소는 절대 포함하지 않는다:
  · ## 헤더 (예: ## STAGE 1 HOOK)
  · 타임코드 (예: [00:00], [HOOK])
  · 마크다운 기호 (**, *, #, -, >)
  · 괄호 지문 (예: (카메라 줌인), [장면: ...])
  · 시각화 메모 (예: DATA_SKETCH_SCENE)
  · 줄바꿈 이중 마크다운
- 오직 말할 수 있는 문장만 작성한다
- 단락 구분은 빈 줄 1개만 사용한다

규칙 3. 숫자·연도 표기 (TTS 오독 방지)
...
- 변환 예시:
  ...
  2배       → 두 배
  3분의 1   → 삼분의 일
- 단, 채널명·브랜드명·고유명사는 그대로 유지

규칙 4. 난이도 (반드시 준수)
- 초등학생도 이해할 수 있는
  쉬운 어휘와 친절한 설명을 사용한다
- 전문 용어가 나오면 반드시
  바로 다음 문장에서 쉬운 말로 풀어 설명한다
- 예시:
  '양적 완화'가 나오면
  → '쉽게 말하면, 나라에서 돈을
     더 많이 찍어내는 거예요.'
...
- 아래 구성 방식을 참고하여 작성한다:
  ...
  · 구체적인 숫자와 사례로 신뢰 확보
  · 이야기 형식으로 정보 전달
  · 중간중간 시청자에게 말 거는 어투 사용
    예: '여러분, 혹시 이런 경험 있으신가요?'
        '잠깐, 여기서 중요한 게 있어요.'
        '이 부분이 핵심입니다. 잘 들어주세요.'
  · 자연스러운 구어체 유지
    (격식체와 구어체 적절히 혼용)
...
[대본 작성 규칙]
1. TTS 순수 구어체 대본 — 헤더·타임코드·마크다운 기호 절대 금지
2. 실제 말하는 대사만 서술 (따옴표 없이)
3. 괄호 지시문 절대 금지 (예: (화면: ...) 불가)
4. 07:00 미니훅 대사는 대본 본문에 자연스럽게 삽입
5. 목표 분량: 앞부분 5,000자 이상 (부족 시 즉시 보완)
6. 단락 구분은 빈 줄 1개만 사용

[OUTPUT RULE]
- TTS에 바로 입력되는 순수 구어체 텍스트만 반환
- 헤더(##), 타임코드([00:00]), 마크다운(*,#,-,>), 괄호 지문 절대 금지
- 숫자·연도는 한글 독음으로 표기 (예: 2025년→이천이십오 년)
- 5,000자 이상 작성 후 종료
"""
...
2. 실제 말하는 대사만 서술 (따옴표 없이)
3. 괄호 지시문 절대 금지 (예: (화면: ...) 불가)
4. 미니훅 대사는 대본 본문에 자연스럽게 삽입
5. STAGE 8 끝에 구독·좋아요·다음 영상 멘트 포함
6. 목표 분량: 뒷부분 5,000자 이상 (부족 시 즉시 보완)
7. 단락 구분은 빈 줄 1개만 사용

[VALIDATION CHECKLIST]
출력 전 아래 항목을 반드시 자가 점검한다:

[TTS 품질 검증]
✅ 헤더·타임코드·마크다운이 전혀 포함되지 않았는가?
✅ 모든 숫자가 한글 독음으로 표기되었는가?
✅ 전문 용어 뒤에 쉬운 설명이 바로 이어지는가?
✅ 앞부분 5,000자 이상인가?
✅ 뒷부분 5,000자 이상인가?
✅ 전체 10,000자 이상인가?
✅ 초등학생이 들어도 이해되는 수준인가?
✅ 시청자에게 말 거는 문장이 중간중간 포함되어 있는가?
